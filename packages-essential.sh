...
if [ -z $(grep AutoSambaAdd /etc/fstab ) ]; then
    ...
    echo "//192.168.1.101/matze /home/matze/mnt/matze cifs noauto,x-systemd.automount,credentials=/etc/samba/credentials/share,vers=3.0,iocharset=utf8,uid=matze,gid=users,_netdev 0 0" >> /etc/fstab
    echo "//192.168.1.101/common /home/matze/mnt/common cifs noauto,x-systemd.automount,credentials=/etc/samba/credentials/share,vers=3.0,iocharset=utf8,uid=matze,gid=users,_netdev 0 0" >> /etc/fstab
fi
